...
import { publishVideoPost } from '@/lib/services/threads-publisher.service'
import { getOrBuildThreadsPostUrl } from '@/lib/services/threads.service'
import { validateMediaUrl } from '@/lib/utils/content-parser'

export class VideoPostPublishingStrategy implements IPostPublishingStrategy {
  readonly contentType = ContentType.VIDEO

  canHandle(post: Post): boolean {
    return post.contentType === ContentType.VIDEO
  }
  ...
  async validate(context: PublishContext): Promise<ValidationErrorMessage[]> {
    ...
    if (!mediaItem?.url) {
      errors.push({ field: 'media', message: 'Video URL is required' })
    } else {
  ...
  async publish(context: PublishContext): Promise<PublishResult> {
    ...
    const platformPostId = await publishVideoPost(
